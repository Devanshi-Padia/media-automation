...
class TestWriteUser:
    ...
    async def test_create_user_success(self, mock_db, sample_user_data, sample_user_read):
        ...
        with patch("src.app.api.v1.users.crud_users") as mock_crud:
            ...
            mock_user = Mock(id=1)
            mock_user.email = user_create.email  # Ensure email is a real string
            mock_crud.create = AsyncMock(return_value=mock_user)
            ...
            with patch("src.app.api.v1.users.get_password_hash") as mock_hash:
                ...
                with patch("aiosmtplib.send") as mock_send_email:
...
class TestPatchUser:
    ...
    async def test_patch_user_success(self, mock_db, current_user_dict, sample_user_read):
        ...
        user_update = UserUpdate(
            ...
            email=sample_user_read.email,
            profile_image_url=sample_user_read.profile_image_url
        )
    ...
    async def test_patch_user_forbidden(self, mock_db, current_user_dict, sample_user_read):
        ...
        username = "differentuser"  # Use a valid username per model's pattern
