...
from ...core.utils.email import send_verification_email, create_email_token
...
async def write_user(
    ...
    # Removed email verification for local development
    # token = create_email_token(created_user.email, settings.SECRET_KEY.get_secret_value())
    # await send_verification_email(created_user.email, token)
...
async def verify_email(
    ...
    try:
        ...
        # user may be a dict or a Pydantic model, so use .get or dict access
        is_verified = False
        if isinstance(user, dict):
            is_verified = user.get("is_email_verified", False)
        else:
            # fallback for Pydantic model
            is_verified = getattr(user, "is_email_verified", False)
        if is_verified:
            return {"message": "Email already verified"}
        await crud_users.update(db=db, object={"is_email_verified": True}, email=email)
