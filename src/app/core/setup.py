...
async def create_redis_cache_pool() -> None:
    ...
    cache.client = redis.Redis(connection_pool=cache.pool)  # type: ignore


async def close_redis_cache_pool() -> None:
    if cache.client is not None:
        await cache.client.close()  # type: ignore
...
def lifespan_factory(
    ...
    async def lifespan(app: FastAPI) -> AsyncGenerator:
        ...
        try:
            # Temporarily disabled Redis cache for development
            if isinstance(settings, RedisCacheSettings):
                await create_redis_cache_pool()

            # Temporarily disabled Redis rate limiter for development
            ...
            # Temporarily disabled Redis cleanup for development
            if isinstance(settings, RedisCacheSettings):
