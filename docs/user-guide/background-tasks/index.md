...
Scheduled posts are now handled entirely by an internal ARQ background worker. When you schedule a post (via the `/schedule_post` endpoint), it is stored in the database and will be automatically published by a periodic background task. No external cron service is required.

- The ARQ worker checks for due scheduled posts every minute and publishes them.
- You can manually trigger a scheduled post for admin/debugging via the `/execute_scheduled_post/{post_id}` endpoint.

This approach ensures all scheduling is managed within your infrastructure, increasing reliability and reducing external dependencies.
